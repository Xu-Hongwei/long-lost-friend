...
const plotRows = computed(() => [
  ...
  ["\u672c\u8f6e\u4efb\u52a1", valueOrEmpty(props.turnContext?.turnMission)],
  ["\u4efb\u52a1\u5019\u9009", valueOrEmpty(props.turnContext?.turnMissionCandidates?.map((item) => `${item.mission || "unknown"}(${item.confidence ?? 0})`).join(" / "))],
  ["\u672c\u5730\u62a4\u680f", valueOrEmpty(props.turnContext?.localGuards?.join(" / "))],
  ["\u538b\u529b\u5206\u6876", `relation ${props.turnContext?.plotRelationshipPressure ?? 0} / scene ${props.turnContext?.plotScenePressure ?? 0} / open ${props.turnContext?.plotOpenLoopPressure ?? 0} / event ${props.turnContext?.plotEventPressure ?? 0} / silence ${props.turnContext?.plotSilencePressure ?? 0}`],
